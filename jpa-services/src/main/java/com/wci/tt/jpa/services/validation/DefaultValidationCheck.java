...
public class DefaultValidationCheck extends AbstractValidationCheck {
  ...
  public ValidationResult validate(Atom atom) {
    ...
    if (atom.getName().length() > 0
      ...
      result.getErrors()
          .add("Atom name contains leading whitespace: " + atom.getName());
    }
    ...
    if (atom.getName().length() > 0 && Character
      ...
      result.getErrors()
          .add("Atom name contains trailing whitespace: " + atom.getName());
    }
    ...
    if (matcher.find()) {
      result.getErrors()
          .add("Atom name contains duplicate whitespace: " + atom.getName());
    }
    ...
    if (atom.getName().indexOf("\t") != -1 || atom.getName().indexOf("\r") != -1
        ...
        || atom.getName().indexOf("\u200B") != -1) {
      result.getErrors()
          .add("Atom name contains invalid whitespace: " + atom.getName());
    }
