      ...
      for (final DataContext outputContext : providerMap.get(provider)) {
        ...
        for (final ConverterHandler converter : converterMap
          ...
          if (provider.isPreCheckValid(record)) {
            ...
            // Having the results generated by the provider's process() method,
            // filter out content that is invalid based on either input string
            processedResults = provider.filterResults(processedResults, record);
