...
ttApp
  .controller(
    'SourceDataCtrl',
    function($scope, $http, $q, $timeout, NgTableParams, sourceDataService,
      utilService, securityService, gpService) {
      ...
      function refreshFileTables() {
        ...
        var currentFileIds = $scope.currentSourceData.sourceDataFiles
          .map(function(item) {
            return item.id;
          })
      ...
      $scope.saveSourceData = function(sourceData) {
        ...
        sourceDataService.updateSourceData(sourceData).then(
        // Success
        function(response) {
          sourceData = response;
        });
